...
def disp() :
    ...
    glLoadIdentity()
    camera.setLens(30, 1.0, 0.1, 100.0)     #glutPerspective(fov, asp, near, far) from Lect05. page 30
    camera.applyLens()
    ...
    camera.setCameraLoc(np.array([0.0, 50.0, 0.0]))
    camera.setCamera(np.array([0.0,50.0,0.0]), np.array([0.0,0.0,0.0]), np.array([0.0,0.0,1.0]))    #glutLookup(loc, tar, up) from Lect05. page 38
    
    camera.applyCamera()
    
    #original gluLookAt(0,50,0, 0,0,0, 0,0,1)
...
def main():
    ...
    glutCreateWindow(b"Lab04-2 Use Camera Lens and Location")
